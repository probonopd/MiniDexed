...
static CColor colors[CMIDIDevice::TChannel::Disabled + 1] = {
	{0x7F, 0x00, 0x00},
	{0x7F, 0x16, 0x00},
	{0x7F, 0x32, 0x00},
	{0x7F, 0x48, 0x00},
...
private:
	...
	void SetPadColor (TBankID BankID, TPadID PadID, uint8_t r, uint8_t g, uint8_t b);
	...
	:m_pSynthesizer (pSynthesizer), m_pKeyboard (pKeyboard)
{
	...
	static TMIDIRouteMap map[] = {
		...
		// {0, 0, MIDI_CONTROL_CHANGE, 111, 0xFF, 0, MIDI_CONTROL_CHANGE, MIDI_CC_DETUNE_LEVEL, 0xFF}, // Knob6
		// {0, 0, MIDI_CONTROL_CHANGE, 116, 0xFF, 0, MIDI_CONTROL_CHANGE, MIDI_CC_DETUNE_LEVEL, 0xFF}, // Knob7
		...
		{0, 9, MIDI_NOTE_ON, 44, 0xFF, 0, MIDI_NOTE_ON, 0, 0x7F, .bSkip=true, .bToggle=true, .pFun=s_EnableTG, .pData=this}, // BankB Pad1
		{0, 9, MIDI_NOTE_OFF, 44, 0xFF, 0, MIDI_NOTE_ON, 0, 0xFF, .bSkip=true}, // BankB Pad1
		{0, 9, MIDI_NOTE_ON, 45, 0xFF, 0, MIDI_NOTE_ON, 1, 0x7F, .bSkip=true, .bToggle=true, .pFun=s_EnableTG, .pData=this}, // BankB Pad2
		{0, 9, MIDI_NOTE_OFF, 45, 0xFF, 0, MIDI_NOTE_ON, 1, 0xFF, .bSkip=true}, // BankB Pad2
		{0, 9, MIDI_NOTE_ON, 46, 0xFF, 0, MIDI_NOTE_ON, 2, 0x7F, .bSkip=true, .bToggle=true, .pFun=s_EnableTG, .pData=this}, // BankB Pad3
		{0, 9, MIDI_NOTE_OFF, 46, 0xFF, 0, MIDI_NOTE_ON, 2, 0xFF, .bSkip=true}, // BankB Pad3
		{0, 9, MIDI_NOTE_ON, 47, 0xFF, 0, MIDI_NOTE_ON, 3, 0x7F, .bSkip=true, .bToggle=true, .pFun=s_EnableTG, .pData=this}, // BankB Pad4
		{0, 9, MIDI_NOTE_OFF, 47, 0xFF, 0, MIDI_NOTE_ON, 3, 0xFF, .bSkip=true}, // BankB Pad4
		{0, 9, MIDI_NOTE_ON, 48, 0xFF, 0, MIDI_NOTE_ON, 4, 0x7F, .bSkip=true, .bToggle=true, .pFun=s_EnableTG, .pData=this}, // BankB Pad5
		{0, 9, MIDI_NOTE_OFF, 48, 0xFF, 0, MIDI_NOTE_ON, 4, 0xFF, .bSkip=true}, // BankB Pad5
		{0, 9, MIDI_NOTE_ON, 49, 0xFF, 0, MIDI_NOTE_ON, 5, 0x7F, .bSkip=true, .bToggle=true, .pFun=s_EnableTG, .pData=this}, // BankB Pad6
		{0, 9, MIDI_NOTE_OFF, 49, 0xFF, 0, MIDI_NOTE_ON, 5, 0xFF, .bSkip=true}, // BankB Pad6
		{0, 9, MIDI_NOTE_ON, 50, 0xFF, 0, MIDI_NOTE_ON, 6, 0x7F, .bSkip=true, .bToggle=true, .pFun=s_EnableTG, .pData=this}, // BankB Pad7
		{0, 9, MIDI_NOTE_OFF, 50, 0xFF, 0, MIDI_NOTE_ON, 6, 0xFF, .bSkip=true}, // BankB Pad7
		{0, 9, MIDI_NOTE_ON, 51, 0xFF, 0, MIDI_NOTE_ON, 7, 0x7F, .bSkip=true, .bToggle=true, .pFun=s_EnableTG, .pData=this}, // BankB Pad8
		{0, 9, MIDI_NOTE_OFF, 51, 0xFF, 0, MIDI_NOTE_ON, 7, 0xFF, .bSkip=true}, // BankB Pad8
		{0xFF}, // Sentine
	};

	m_pKeyboard->SetMIDIRouteMap (map);

	m_pKeyboard->Send (pInit, sizeof pInit, 0);
	DisplayWrite ("MiniDexed", "", "On MiniLab 3", 0, 0);

	SetPadColor (BankA, PortamentoPad, 0x11, 0x11, 0x3f);
	SetPadColor (BankA, SostenutoPad, 0x3f, 0x11, 0x3f);
	SetPadColor (BankA, SustainPad, 0x11, 0x3f, 0x11);
	SetPadColor (BankA, SoundOffPad, 0x3f, 0x11, 0x11);
...
void CMiniLab3DawConnection::SetPadColor (TBankID BankID, TPadID PadID, uint8_t r, uint8_t g, uint8_t b)
{
...
{
	CColor color = colors[m_pSynthesizer->GetTGParameter (CMiniDexed::TGParameterMIDIChannel, nTG)];
	...
	SetPadColor (BankB, (TPadID)nTG, color.r, color.g, color.b);
}
...
void CMiniLab3DawConnection::UpdateState ()
{
	...
	SetPadColor (BankA, PortamentoPad, 0x11, 0x11, m_pSynthesizer->GetTGParameter (CMiniDexed::TGParameterPortamentoMode, 0) ? 0x7f : 0x3f);
...
void CMiniLab3DawConnection::ShowNewCCValue (u8 ucCh, u8 ucCC, u8 ucValue)
{
	...
	ArturiaShowNewCCValue (m_pKeyboard, pEncoder, ucCh, ucCC, ucValue);

	switch (ucCC)
	{
	case MIDI_CC_PORTAMENTO:
		SetPadColor (BankA, PortamentoPad, 0x11, 0x11, ucValue ? 0x7f : 0x3f);
		break;
	case MIDI_CC_SOSTENUTO:
		SetPadColor (BankA, SostenutoPad, ucValue ? 0x7f : 0x3f, 0x11, ucValue ? 0x7f : 0x3f);
		break;
	case MIDI_CC_BANK_SUSTAIN:
		SetPadColor (BankA, SustainPad, 0x11, ucValue ? 0x7f : 0x3f, 0x11);
		break;
	case MIDI_CC_ALL_SOUND_OFF:
		SetPadColor (BankA, SoundOffPad, ucValue ? 0x7f : 0x3f, 0x11, 0x11);
		break;
	}
}
	...
	:m_pSynthesizer (pSynthesizer), m_pKeyboard (pKeyboard)
{
	...
	static TMIDIRouteMap map[] = {
		...
		// {0, 0, MIDI_CONTROL_CHANGE, 103, 0xFF, 0, MIDI_CONTROL_CHANGE, MIDI_CC_PAN_POSITION, 0xFF}, // Knob8
		// {0, 0, MIDI_CONTROL_CHANGE, 104, 0xFF, 0, MIDI_CONTROL_CHANGE, MIDI_CC_PAN_POSITION, 0xFF}, // Knob9*/
		{1, 0, MIDI_CONTROL_CHANGE, 60, 0xFF, 0, MIDI_CONTROL_CHANGE, 60, 0xFF}, // Main rotate
		{1, 0, MIDI_NOTE_ON, 84, 0xFF, 0, MIDI_CONTROL_CHANGE, 63, 0xFF}, // Main click
