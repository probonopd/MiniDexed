...
public:
	...
	virtual void UpdateState (CMiniDexed *pSynthesizer, CMIDIKeyboard *pKeyboard) = 0;
...
public:
	...
	void UpdateState (CMiniDexed *pSynthesizer, CMIDIKeyboard *pKeyboard) override;
...
CMiniLab3DawConnection::CMiniLab3DawConnection (CMiniDexed *pSynthesizer, CMIDIKeyboard *pKeyboard, CConfig *pConfig, CUserInterface *pUI)
{
	...
	SetPadColor (pKeyboard, BankA, PortamentoPad, 0x11, 0x11, 0x3f);
	SetPadColor (pKeyboard, BankA, SostenutoPad, 0x3f, 0x11, 0x3f);
	SetPadColor (pKeyboard, BankA, SustainPad, 0x11, 0x3f, 0x11);
	SetPadColor (pKeyboard, BankA, SoundOffPad, 0x3f, 0x11, 0x11);

	UpdateState (pSynthesizer, pKeyboard);
...
void CMiniLab3DawConnection::DisplayWrite (CMIDIKeyboard *pKeyboard, const char *pMenu, const char *pParam, const char *pValue, bool bArrowDown, bool bArrowUp)
{
	...
	ArturiaDisplayWrite (pKeyboard, pHdr, sizeof pHdr, 18, pMenu, pParam, pValue);
}
...
void CMiniLab3DawConnection::UpdateState (CMiniDexed *pSynthesizer, CMIDIKeyboard *pKeyboard)
{
	...
	UpdateEncoder (pKeyboard, 4, maplong(pSynthesizer->GetTGParameter (CMiniDexed::TGParameterPortamentoTime, 0), 0, 99, 0, 127));
	...
	SetPadColor (pKeyboard, BankA, PortamentoPad, 0x11, 0x11, pSynthesizer->GetTGParameter (CMiniDexed::TGParameterPortamentoMode, 0) ? 0x7f : 0x3f);
}
...
void CMiniLab3DawConnection::ShowNewCCValue (CMIDIKeyboard *pKeyboard, u8 ucCh, u8 ucCC, u8 ucValue)
{
	...
	{
	case MIDI_CC_PORTAMENTO:
		SetPadColor (pKeyboard, BankA, PortamentoPad, 0x11, 0x11, ucValue ? 0x7f : 0x3f);
		break;
	case MIDI_CC_SOSTENUTO:
		SetPadColor (pKeyboard, BankA, SostenutoPad, ucValue ? 0x7f : 0x3f, 0x11, ucValue ? 0x7f : 0x3f);
		break;
	case MIDI_CC_BANK_SUSTAIN:
...
void CKeyLabEs3DawConnection::UpdateState (CMiniDexed *pSynthesizer, CMIDIKeyboard *pKeyboard)
{
	UpdateEncoder (pKeyboard, 0, maplong(pSynthesizer->GetTGParameter (CMiniDexed::TGParameterCutoff, 0), 0, 99, 0, 127));
	UpdateEncoder (pKeyboard, 1, maplong(pSynthesizer->GetTGParameter (CMiniDexed::TGParameterResonance, 0), 0, 99, 0, 127));
	UpdateEncoder (pKeyboard, 2, maplong(pSynthesizer->GetTGParameter (CMiniDexed::TGParameterReverbSend, 0), 0, 99, 0, 127));
	...
	UpdateEncoder (pKeyboard, 5, maplong(pSynthesizer->GetTGParameter (CMiniDexed::TGParameterPortamentoTime, 0), 0, 99, 0, 127));
}
...
CKeyLab2DawConnection::CKeyLab2DawConnection (CMiniDexed *pSynthesizer, CMIDIKeyboard *pKeyboard, CConfig *pConfig, CUserInterface *pUI)
{
	...
	pKeyboard->Send (pInit, sizeof pInit, 0);
	DisplayWrite (pKeyboard, "MiniDexed", "", "On KeyLab 2", 0, 0);
...
void CKeyLab2DawConnection::DisplayWrite (CMIDIKeyboard *pKeyboard, const char *pMenu, const char *pParam, const char *pValue, bool bArrowDown, bool bArrowUp)
{
	static const uint8_t pHdr[] = {0xF0, 0x00, 0x20, 0x6B, 0x7F, 0x42, 0x04, 0x00, 0x60, 0x01};
	ArturiaDisplayWrite (pKeyboard, pHdr, sizeof pHdr, 16, pMenu, pParam, pValue);
}

void CKeyLab2DawConnection::UpdateState (CMiniDexed *pSynthesizer, CMIDIKeyboard *pKeyboard)
{
}

void CKeyLab2DawConnection::ShowNewCCValue (CMIDIKeyboard *pKeyboard, u8 ucCh, u8 ucCC, u8 ucValue)
{
	//static const uint8_t pEncoder[] = {0x04, 0x01, 0x60};
	//ArturiaShowNewCCValue (pKeyboard, pEncoder, ucCh, ucCC, ucValue);
}
...
{
	if (m_pDAWConnection)
		m_pDAWConnection->UpdateState (m_pSynthesizer, m_pKeyboard);
}
