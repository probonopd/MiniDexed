...
*/
	...
	if (m_pMIDIRouteMap)
		GetRoutedMIDI (m_pMIDIRouteMap, &ucCable, &ucChannel, &ucType, &ucP1, &ucP2, &bSkip, &pFun, &pData);

	if (pFun)
		pFun(pData, ucCable, ucChannel, ucType, ucP1, ucP2);
...
void GetRoutedMIDI (TMIDIRouteMap *map, u8 *pCable, u8 *pCh, u8 *pType, u8 *pP1, u8 *pP2, bool *bSkip, TMidiFun **pFun, void **pData)
{
	assert (map);
	for (TMIDIRouteMap *r = map; r->ucSCable != 0xFF ; r++)
	{
